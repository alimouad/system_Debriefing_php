    ...
    <div class="mx-auto max-w-4xl flex flex-col gap-8">
        <div class="flex flex-col gap-4">
            <nav class="flex items-center gap-2 text-sm text-[#5b8b66]">
                ...
                <span class="material-symbols-outlined text-[16px]">chevron_right</span>
                <span class="font-medium text-[#101912] dark:text-white">Add New</span>
            </nav>
            <div class="flex flex-wrap items-end justify-between gap-4">
                <div>
                    <h1 class="text-3xl md:text-4xl font-black tracking-tight text-[#101912] dark:text-white">
                        Add New Category</h1>
                    <p class="mt-2 text-[#5b8b66]">Create a new category for your blog content.</p>
                </div>
            </div>
        </div>
        <div class="rounded-xl border border-[#e9f1eb] dark:border-[#2a362c] bg-white dark:bg-[#1a261e] shadow-sm">
            <div class="border-b border-[#e9f1eb] dark:border-[#2a362c] px-6 py-4 md:px-8">
                <h3 class="text-lg font-bold text-[#101912] dark:text-white">Category Details</h3>
            </div>
            <form action="<?= route('admin.categories.store') ?>" method="post" class="p-6 md:p-8 flex flex-col gap-6">
                <label class="flex flex-col gap-1.5">
                    ...
                    <input
                        ...
                        placeholder="e.g. Technology" name="name" type="text" />
                </label>
